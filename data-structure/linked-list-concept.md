...
제네릭을 써서 여러 타입을 받을 수 있도록 한다. 특히 리스트의 반복을 위한 `Iterator` 를 반환하는 메소드는 연결 리스트 각 구현체의 특성에 맞게 구현된 `Iterator` 를 반환해야 한다.
...
head, tail을 별도로 유지하는 경우 첫 노드와 끝 노드에는 $O(1)$로 접근할 수 있지만 아닌 경우 노드의 링크를 타고가며 하나씩 확인해야 하므로 비효율적인 편이다. 그러나 노드의 삽입과 삭제 연산은 링크만 변경하면 되므로 매우 효율적이다.
