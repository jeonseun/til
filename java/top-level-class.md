...
- `A$B.class` - `A` 의 정적 내부 클래스 `B` 의 `.class` 파일
...
`public` top level 클래스가 없다면 소스파일 내 클래스 중 하나의 이름으로 소스파일의 파일명을 결정하면 된다. 물론 캡슐화를 위한 목적이 있는게 아니라면 하나의 소스파일에는 하나의 top level 클래스만 선언하는 것이 바람직하다.