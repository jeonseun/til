...
정적 메서드(static method)는 `static` 키워드가 붙은 메서드로 객체 생성없이 바로 사용할 수 있는 메서드다.
...
정적 메서드는 유틸리티성 메서드를 정의하는데 많이 사용된다. 대표적으로 자바 API 중 수학 관련 클래스인 `Math` 에 정의된 메서드들은 모두 정적 메서드다.
