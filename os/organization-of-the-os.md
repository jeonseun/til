...
OS는 크게 핵심이 되는 커널과 시스템 프로그램 그리고 응용 프로그램으로 구성되며 OS의 서비스를 제공하는 커널의 종류에 따라 몇 가지 구성 방식이 있다.

## 서비스 관점에서의 OS

OS는 자원 할당자, 제어 프로그램 등의 역할을 한다. OS가 제공하는 서비스를 컴퓨터 시스템 구성 요소와 함께 보면 다음과 같다.
...
| 장치 드라이버     | 하드웨어 장치(장치 컨트롤러)와 OS의 상호작용을 위한 인터페이스를 제공한다.                                                   |

## OS 구성 방식

### 모놀리식

가장 단순한 구조로 커널에 모든 기능을 담아 하나의 이진 파일에 구성한다. 커널은 세부적으로 여러 인터페이스와 장치 드라이버로 분리된다.

시스템 콜 그리고 하드웨어 인터페이스 사이의 모든것이 커널에 결합되어 커널 내에 모든 기능이 있으므로 시스템 콜에 대한 오버헤드가 없고 커널 내의 통신 속도가 빠르다. 대신 확장이 어렵다.

### 레이어드

OS를 사용자부터 가장 밑의 하드웨어까지 여러 계층으로 나누어 구성하는 방식이다.

각 계층이 자신의 하위 계층이 제공하는 서비스와 기능만 사용하도록 구성한다. 특정 계층이 올바르게 구현되었다면 이 계층을 쓰는 다른 계층은 해당 계층에서 오류가 발생하지 않는다고 신뢰할 수 있으므로 오류 발생 시 오류 발생 지점을 쉽게 찾을 수 있다. 대신 여러 계층을 통과해야 하므로 오버헤드가 발생한다.
...
서비스를 제공하는 모듈을 커널에 동적으로 연결하기 때문에 새로운 기능의 추가가 쉽다. 따라서 다른 기능을 모듈의 형태로 추가해도 기존 커널을 재컴파일할 필요가 없으며 통신하기 위해 메시지를 전달할 필요가 없다. 더불어 각 모듈이 보호된 인터페이스를 가진다는 점이 장점이다.
